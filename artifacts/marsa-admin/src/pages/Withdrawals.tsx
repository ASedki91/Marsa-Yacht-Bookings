    ...
    <p className="text-xs text-muted-foreground mt-0.5 flex items-start gap-1">
      ...
      <span>
        {entries.map(([k, v], i) => (
          <span key={k}>
            {i > 0 && " · "}
            <span className="text-foreground/60">{labels[k] ?? k}:</span> {v}
          </span>
        ))}
      </span>
    </p>
...
function EarningsBreakdown({ earnings }: { earnings: any[] }) {
  if (!earnings?.length) {
    return (
      <p className="text-xs text-muted-foreground py-1">
        No earnings records found for this host.
      </p>
    );
  }
  ...
  const total = (arr: any[]) =>
    arr.reduce((s, e) => s + parseFloat(e.amountEgp ?? "0"), 0);
      ...
      {available.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-green-400 mb-1">
            Available ({available.length} entries ·{" "}
            {formatEgp(total(available))})
          </p>
      ...
      {pending.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-amber-400 mb-1">
            Pending release ({pending.length} entries ·{" "}
            {formatEgp(total(pending))})
          </p>
          <div className="space-y-1">
            {pending.map((e: any) => (
              <div
      ...
      {withdrawn.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-muted-foreground mb-1">
            Already withdrawn ({withdrawn.length} entries ·{" "}
            {formatEgp(total(withdrawn))})
          </p>
        </div>
      )}
    </div>
...
export default function Withdrawals() {
  ...
  const { data, isLoading, isSuccess } = useAdminListWithdrawals({
    query: { queryKey: getAdminListWithdrawalsQueryKey() },
  });
  useAdminSectionSeen("withdrawals", isSuccess);
    ...
    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {w.hostName ? (
                            <>
                              ...
                              {w.hostEmail && <span> · {w.hostEmail}</span>}
                            </>
                          ) : (
                          ...
                          {w.payoutReference && (
                            <span> · Ref: {w.payoutReference}</span>
                          )}
                        </p>
                    ...
                    {w.status === "withdrawal_requested" && (
                      <div className="flex gap-2 ml-4 shrink-0">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setDialog({ id: w.id, action: "rejected" })
                          }
                        >
                          <XCircle className="w-3 h-3 mr-1" />
                          Reject
                  ...
                  {isExpanded && (
                    <div className="mt-3 pt-3 border-t border-border">
                      <p className="text-xs font-semibold text-muted-foreground flex items-center gap-1 mb-2">
                        ...
                        Earnings Breakdown for this host
                      </p>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {dialog?.action === "paid" ? "Mark as Paid" : "Reject Withdrawal"}
            </DialogTitle>
          </DialogHeader>
          {dialog?.action === "paid" ? (
            <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                process.isPending ||
                (dialog?.action === "rejected" && !rejectReason.trim())
              }
              onClick={() =>
                dialog &&
                process.mutate({
