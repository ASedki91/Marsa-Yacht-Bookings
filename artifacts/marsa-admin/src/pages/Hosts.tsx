...
import {
  useAdminListHosts, useAdminVerifyHost,
...
const statusBadge: Record<string, React.ReactNode> = {
  ...
  verified: <Badge variant="outline" className="text-green-400 border-green-400/40"><CheckCircle className="w-3 h-3 mr-1" />Verified</Badge>,
...
export default function Hosts() {
  ...
  const [dialog, setDialog] = useState<{ id: string; action: "verified" | "rejected" } | null>(null);
  const [reason, setReason] = useState("");
  const { toast } = useToast();
  const qc = useQueryClient();

  const params = statusFilter !== "all" ? { status: statusFilter as any } : {};
  const { data, isLoading } = useAdminListHosts(params, {
  ...
  const verify = useAdminVerifyHost({
    mutation: {
      ...
      onError: () => toast({ title: "Failed to update host", variant: "destructive" }),
    }
  });
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <div className="text-center text-muted-foreground py-16">No hosts found</div>
      ) : (
              ...
              <CardContent className="flex items-center justify-between py-3 px-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-foreground text-sm truncate">{host.displayName || host.businessName || host.id}</span>
                    {statusBadge[host.verificationStatus]}
                  </div>
                  <p className="text-xs text-muted-foreground mt-0.5 truncate">{host.city} · {host.country} · {new Date(host.createdAt).toLocaleDateString()}</p>
                </div>
                {host.verificationStatus === "pending" && (
                  <div className="flex gap-2 ml-4 shrink-0">
                    <Button size="sm" variant="outline" className="text-green-400 border-green-400/40 hover:bg-green-400/10"
                      ...
                      onClick={() => setDialog({ id: host.id, action: "verified" })}>Approve</Button>
                    <Button size="sm" variant="outline" className="text-destructive border-destructive/40 hover:bg-destructive/10"
                      ...
                      onClick={() => setDialog({ id: host.id, action: "rejected" })}>Reject</Button>
                  </div>
                )}
              </CardContent>
            </Card>
      ...
      <Dialog open={!!dialog} onOpenChange={() => { setDialog(null); setReason(""); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{dialog?.action === "verified" ? "Approve Host" : "Reject Host"}</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              variant={dialog?.action === "verified" ? "default" : "destructive"}
              disabled={verify.isPending}
              onClick={() => dialog && verify.mutate({ id: dialog.id, data: { status: dialog.action, reason: reason || undefined } })}
            >
              {verify.isPending ? "Processing..." : dialog?.action === "verified" ? "Approve" : "Reject"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
