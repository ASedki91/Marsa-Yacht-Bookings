...
import { AlertTriangle, CheckCircle, XCircle, ChevronDown, ChevronRight, User, Anchor, CreditCard } from "lucide-react";
  ...
  return (
    <div className="mt-3 pt-3 border-t border-border grid grid-cols-1 sm:grid-cols-3 gap-3">
      <div>
        ...
        {b.guestNationality && <p className="text-xs text-muted-foreground">{b.guestNationality}</p>}
      ...
      <div>
        <p className="text-xs font-semibold text-muted-foreground flex items-center gap-1 mb-1.5">
          <Anchor className="w-3 h-3" />Yacht
        </p>
        <p className="text-xs text-foreground">{b.yachtName ?? `ID: ${b.yachtId?.slice(0, 8)}`}</p>
        <p className="text-xs text-muted-foreground">{b.bookingDate}</p>
        {b.startTime && <p className="text-xs text-muted-foreground">Start: {b.startTime}</p>}
        <p className="text-xs text-muted-foreground font-mono">Booking ID: {b.id?.slice(0, 8)}</p>
      </div>
      <div>
        <p className="text-xs font-semibold text-muted-foreground flex items-center gap-1 mb-1.5">
          <CreditCard className="w-3 h-3" />Payment
        </p>
        <p className="text-xs text-foreground font-semibold">{formatEgp(b.totalAmountEgp)}</p>
        {b.baseAmountEgp && <p className="text-xs text-muted-foreground">Base: {formatEgp(b.baseAmountEgp)}</p>}
        {b.platformFeeEgp && <p className="text-xs text-muted-foreground">Platform fee: {formatEgp(b.platformFeeEgp)}</p>}
        {b.hostEarningsEgp && <p className="text-xs text-muted-foreground">Host earnings: {formatEgp(b.hostEarningsEgp)}</p>}
...
export default function Bookings() {
  ...
  const otherBookings = bookings.filter(b => b.status !== "paid_under_review");
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Select value={statusFilter} onValueChange={v => { setStatusFilter(v); setPage(1); }}>
          ...
          <SelectContent>
            ...
            <SelectItem value="paid_under_review">Pending Host Review</SelectItem>
                      ...
                      >
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <Badge variant="outline" className={`text-xs ${statusColors[b.status] ?? ""}`}>
                            {STATUS_DISPLAY[b.status] ?? b.status?.replace(/_/g, " ")}
                          </Badge>
