...
function StatCard({ icon, label, value, sub, accent }: {
  ...
  return (
    <View style={[
      ...
      <Ionicons name={icon as any} size={22} color={accent ? colors.light.gold : c.primary} />
      <Text style={[sStyles.value, { color: accent ? "#fff" : c.foreground }]}>{value}</Text>
      <Text style={[sStyles.label, { color: accent ? "#CBD5E1" : c.mutedForeground }]}>{label}</Text>
      {sub && <Text style={[sStyles.sub, { color: accent ? colors.light.gold : c.primary }]}>{sub}</Text>}
    </View>
  );
}

const sStyles = StyleSheet.create({
  card: { flex: 1, borderRadius: 16, borderWidth: 1, padding: 16, gap: 4, alignItems: "flex-start", minHeight: 100 },
...
function BookingRow({ booking, onPress }: { booking: any; onPress: () => void }) {
  ...
  return (
    <Pressable
      ...
      onPress={onPress}
    >
      <View style={[bStyles.dot, { backgroundColor: statusColors[status] ?? c.muted }]} />
      <View style={{ flex: 1 }}>
        <Text style={[bStyles.guest, { color: c.foreground }]} numberOfLines={1}>
          {booking.guest?.firstName ?? "Guest"} {booking.guest?.lastName ?? ""}
        </Text>
        <Text style={[bStyles.yacht, { color: c.mutedForeground }]} numberOfLines={1}>
          {booking.yacht?.title ?? booking.yacht?.name ?? "Yacht"}
        </Text>
      </View>
      <View style={{ alignItems: "flex-end", gap: 2 }}>
        <View style={[bStyles.badge, { backgroundColor: (statusColors[status] ?? "#6B7280") + "20" }]}>
          <Text style={[bStyles.badgeText, { color: statusColors[status] ?? c.mutedForeground }]}>
            {status.charAt(0).toUpperCase() + status.slice(1)}
          </Text>
        </View>
        <Text style={[bStyles.amount, { color: c.foreground }]}>
          EGP {booking.totalEgp ?? "—"}
        </Text>
      </View>
    </Pressable>
  );
...
export default function DashboardScreen() {
  ...
  const { data: earningsData, isLoading: earningsLoading, refetch: refetchEarnings } = useGetHostEarnings();
  const { data: bookingsData, isLoading: bookingsLoading, refetch: refetchBookings } = useListMyBookings({ role: "host", page: 1 });
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <View>
          <Text style={[styles.greeting, { color: c.mutedForeground }]}>Host Dashboard</Text>
        ...
        <>
          ...
          {pendingBookings.length > 0 && (
            <View style={[styles.alertBanner, { backgroundColor: "#FEF3C7", borderColor: "#F59E0B" }]}>
              ...
              <Text style={[styles.alertText, { color: "#92400E" }]}>
                You have {pendingBookings.length} booking{pendingBookings.length > 1 ? "s" : ""} awaiting your confirmation.
              </Text>
              <Pressable onPress={() => router.push("/(home)/(tabs)/bookings")}>
                <Text style={[styles.alertLink, { color: "#D97706" }]}>Review →</Text>
              </Pressable>
            </View>
          )}
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={[styles.sectionTitle, { color: c.foreground }]}>Recent Bookings</Text>
              <Pressable onPress={() => router.push("/(home)/(tabs)/bookings")}>
            ...
            {recentBookings.length === 0 ? (
              <View style={[styles.emptyCard, { backgroundColor: c.card, borderColor: c.border }]}>
                ...
                <Text style={[styles.emptyText, { color: c.mutedForeground }]}>No bookings yet</Text>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.actionsGrid}>
              {[
                { icon: "boat-outline", label: "My Yachts", route: "/(home)/(tabs)/yachts" as const },
                { icon: "add-circle-outline", label: "List Yacht", route: "/(home)/new-yacht" as const },
                { icon: "cash-outline", label: "Earnings", route: "/(home)/(tabs)/earnings" as const },
                { icon: "person-outline", label: "Profile", route: "/(home)/(tabs)/profile" as const },
              ].map((action) => (
                <Pressable
                  key={action.label}
                ...
                >
                  <Ionicons name={action.icon as any} size={22} color={c.primary} />
...
const styles = StyleSheet.create({
  ...
  alertBanner: { flexDirection: "row", alignItems: "center", gap: 10, padding: 14, borderRadius: 12, borderWidth: 1 },
  ...
  bookingList: { gap: 8 },
  emptyCard: { padding: 24, borderRadius: 16, borderWidth: 1, alignItems: "center", gap: 8 },
  ...
  actionCard: { width: "47%", padding: 16, borderRadius: 14, borderWidth: 1, gap: 8, alignItems: "center" },
  actionLabel: { fontSize: 13, fontFamily: "Inter_500Medium", textAlign: "center" },
  feeNote: { flexDirection: "row", gap: 10, padding: 14, borderRadius: 12, borderWidth: 1, alignItems: "flex-start" },
  feeText: { flex: 1, fontSize: 12, fontFamily: "Inter_400Regular", lineHeight: 17 },
});
